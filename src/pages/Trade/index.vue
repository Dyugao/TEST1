  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        <p @click="changeDefaultAddress(address, userAddressList)">
      ...
      <div class="line"></div>
      <h5 class="pay" style="position: absolute; padding-top: 20px">
        送货清单
      </h5>
      <div class="way">
        <h5>配送方式</h5>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ userGetAdress.fullAddress }}</span>
        收货人：<span>{{ userGetAdress.consignee }}</span>
        <span>{{ userGetAdress.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters(["detailArrayList","userAddressList", "orderDetailVoList"]),
    //总价
    ...mapState({
  ...
  methods: {
    changeDefaultAddress(address, userAddressList) {
    ...
    async submitTrade() {
      ...
      let data = {
        consignee: this.userGetAdress.consignee,//最终收件人姓名
        consigneeTel: this.userGetAdress.phoneNum,//最终收件人号码
        deliveryAddress: this.userGetAdress.fullAddress,//最终收件人地址
        paymentWay: "ONLINE",//支付方式
        orderComment:this.orderComment,//买家的留言信息
        orderDetailList: this.detailArrayList//商品清单
      };
       //需要带参数的：tradeNo
      let result=await this.$API.reqSubmitTrade(tradeNo, data);
      if(result.code==200){
